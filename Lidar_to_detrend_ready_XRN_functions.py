...
#INPUTS
direct = r"Z:\users\xavierrn\SoCoast_Final_ResearchFiles\COMID17607553"
cell_size = 0.7
...
def lidar_footptint(direct, spatial_ref):
    ...
    try:
        ...
        for f in files_in_direct:
            if f[-4:] == ".laz":
                print("%slaszip.exe -i %s\\%s -o %s\\%s.las" % (lastooldirect, direct, f, laspath, f[:-4]))
                cmd("%slaszip.exe -i %s\\%s -o %s\\%s.las" % (lastooldirect, direct, f, laspath, f[:-4]))
...
def detrend_prep(raster_name, flow_polygon, spatial_ref, spatial_extent):
    ...
    station_points = arcpy.Intersect_analysis([station_lines, centerline], out_feature_class=direct + "\\station_points_40_smooth.shp", join_attributes="ALL", output_type="POINT")
    station_points = arcpy.MultipartToSinglepart_management(station_points, direct +"\\raw_station_points_40_smooth.shp")
    station_points = arcpy.AddXY_management(station_points)
    elevation_table = arcpy.ExtractValuesToTable_ga(station_points, in_rasters=raster_name, out_table=(direct + "\\sp_elevation_table_40_smooth.dbf"))
    ...
    elevation_table = arcpy.TableToExcel_conversion(station_points, direct + "\\XY_elevation_table_40_smooth.xls")

    print("Station points shapefile at: " + str(station_points))
    print("Elevation table at: " + str(elevation_table))

lidar_footptint(direct, spatial_ref)
