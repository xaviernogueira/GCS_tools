...
table_directory = out_folder + "\\gcs_ready_tables"
##################
...
def GCS_plotter(table_directory):
    ...
    list_of_csv_tables = [f for f in listdir(table_directory) if isfile(join(table_directory, f))]
    ...
    for table in list_of_csv_tables:
        ...
        wide_bar_df = table_df.loc[table_df['code'] == 1]
        nozzle_df = table_df.loc[table_df['code'] == 2]
        normal_df = table_df.loc[table_df['code'] == 0]
        pool_df = table_df.loc[table_df['code'] == 1]
        oversized_df = table_df.loc[table_df['code'] == -2]

        # List format ex: nozzle_list_of_lists = [dist, Zs, Ws, WsZs]
        ws_list = []
        zs_list = []
        zs_ws_list = []
        dist_down_list = []
        code_list = []
        listoflists = [dist_down_list, ws_list, zs_list, zs_ws_list, code_list]

        normal_list_of_lists = []
        wide_list_of_lists = []
        pool_list_of_lists = []
        nozzle_list_of_lists = []
        oversized_list_of_lists = []


GCS_plotter(table_directory=table_directory)
