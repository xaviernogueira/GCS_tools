...
def landforms(table, zs_field='Z_s', ws_field='W_s', na=-9999):
    ...
    df['wide_bar'] = [zs * ws if (abs(zs * ws) > 0.5 and zs > 0.5 and ws > 0.5) else na for zs, ws in
                      zip(df[zs_field], df[ws_field])]
    df['const_pool'] = [zs * ws if (abs(zs * ws) > 0.5 and zs < -0.5 and ws < -0.5) else na for zs, ws in
                        zip(df[zs_field], df[ws_field])]
    df['nozzle'] = [zs * ws if (abs(zs * ws) > 0.5 and zs > 0.5 and ws < -0.5) else na for zs, ws in
                    zip(df[zs_field], df[ws_field])]
    df['oversized'] = [zs * ws if (abs(zs * ws) > 0.5 and zs < 0.5 and ws > 0.5) else na for zs, ws in
                       zip(df[zs_field], df[ws_field])]
